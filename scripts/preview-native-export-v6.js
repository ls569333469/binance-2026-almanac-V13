...
async function generatePreview() {
    console.log(':: NATIVE EXPORT PREVIEW (V6 - Gradient Restoration) :: Initializing...');
    ...
    try {
        ...
        console.log('Applying Micro-Universe V6 (Explicit Gradient)...');
        ...
        await page.evaluate(() => {
            ...
            // We do this to GUARANTEE the gradient is present, bypassing any computed style anomalies.
            // Copied from index.html:
            const exactGradient = 'radial-gradient(circle at 50% 0%, #151921 0%, #000 85%), radial-gradient(circle at 85% 30%, rgba(240, 185, 11, 0.08) 0%, transparent 50%)';
            const exactBgColor = '#000';
            ...
            bgLayer.style.backgroundAttachment = 'fixed'; // Keep viewport sync
            bgLayer.style.backgroundPosition = 'center top'; // Critical for matching the 'top-lighter' look
        ...
        console.log('Capturing Snapshot V6...');
        await wrapperHandle.screenshot({
            path: path.join(CONFIG.outputDir, 'preview-micro-universe-006.png'),
            omitBackground: true
        });

        console.log(`\nPREVIEW GENERATED: ${path.join(CONFIG.outputDir, 'preview-micro-universe-006.png')}`);
